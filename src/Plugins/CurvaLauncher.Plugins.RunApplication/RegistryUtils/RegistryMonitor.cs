...
namespace CurvaLauncher.Plugins.RunApplication.RegistryHelper
{
    ...
    public class RegistryMonitor : IDisposable
    {
        ...
        [MemberNotNull(nameof(_registryHive))]
        [MemberNotNull(nameof(_registrySubName))]
        ...
        private void ThreadLoop()
        {
            ...
            try
            {
                ...
                {
                    var safeWaitHandle = _eventNotify.SafeWaitHandle;
                    var handle = safeWaitHandle.DangerousGetHandle();
                    result = RegNotifyChangeKeyValue(registryKey, true, _regFilter, handle, true);
                    if (result != 0)
                        throw new Win32Exception(result);
